...
type planet = {
    nome: string,
        coordenadas: planetCoordinates,
        status:planetSituation,
        satelites:string[]
}

let planetList:planet[] = [];

function savePlanet ( nome:string, coordenadas:planetCoordinates,
status:planetSituation){
...
function statusPlanetUpdate(planet:planet,  status:planetSituation) {
    planet.status = status;
    alert(`planeta ${planet.nome} salvo com sucesso!`)
}

function addSatelitePlanet(name:string, planet:planet) {
    planet.satelites.push(name);
    alert(`O satelite ${name} foi adicionado ao planeta ${planet.nome}`)
}

function removeSatelitePlanet(name:string, planet:planet){
