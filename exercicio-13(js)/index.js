...
function restartGame() {
  ...
  boardRegions.forEach(function (element) {
    if (element.classList.value == 'win') {
      initializeGame()
    }
  })
}
...
// Adiciona o evento no botão que reinicia o jogo
...
document.getElementById('start').addEventListener('click', function () {
  if (document.getElementById('player1').value != '' && document.getElementById('player2').value != '') {
    boardRegions.forEach(function (element) {
      if (element.classList.value == 'win') {
        alert(`Clique em reinciar para jogar novamente!`)
        throw console.error('jogo encerrado');
      }
      initializeGame()
    })
  } else {
    alert(`favor inserir nome dos jogadores!!`);
  }
})